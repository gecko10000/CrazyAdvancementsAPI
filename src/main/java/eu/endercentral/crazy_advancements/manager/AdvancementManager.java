...
public final class AdvancementManager {
    ...
    private static HashMap<NamespacedKey, AdvancementManager> accessibleManagers = new HashMap<>();
     ...
     */
    public static AdvancementManager getAccessibleManager(NamespacedKey name) {
        return accessibleManagers.get(name);
    }
     ...
     */
    public AdvancementManager(NamespacedKey name, Player... players) {
        this.name = name;
    ...
    public void removePlayer(Player player) {
        ...
        List<NamespacedKey> removedAdvancements = new ArrayList<>();
    ...
    public void addAdvancement(Advancement... addedAdvancements) {
        ...
        HashSet<NamespacedKey> updatedTabs = new HashSet<>();
        ...
        for (NamespacedKey tab : updatedTabs) {
    ...
    public void updateAdvancement(Advancement... updatedAdvancements) {
        Set<NamespacedKey> updatedTabs = new HashSet<>();
        List<Advancement> remainingAdvancements = new ArrayList<>();
        List<NamespacedKey> remainingNames = new ArrayList<>();
        ...
        if (remainingAdvancements.size() > 0) {
            for (Player player : getPlayers()) {
                NamespacedKey activeTab = CrazyAdvancementsAPI.getActiveTab(player);
    ...
    public void removeAdvancement(Advancement... removedAdvancements) {
        List<NamespacedKey> nonDuplicates = new ArrayList<>();
     ...
     */
    public Advancement getAdvancement(NamespacedKey name) {
    ...
    public void makeAccessible() {
        if (accessibleManagers.containsKey(name)) {
        ...
        accessibleManagers.put(name, this);
    }
    ...
    public void resetAccessible() {
        Iterator<NamespacedKey> it = accessibleManagers.keySet().iterator();
        while (it.hasNext()) {
            NamespacedKey name = it.next();
     ...
     */
    public void updateTab(Player player, NamespacedKey tab) {
        List<Advancement> advancements = new ArrayList<>();
        List<NamespacedKey> names = new ArrayList<>();
        for (Advancement advancement : getAdvancements()) {
            if (advancement.getTab().equals(tab) && advancement.getDisplay().isVisible(player, advancement)) {
                advancements.add(advancement);
    ...
    public void loadProgress(UUID uuid, SaveFile saveFile, Advancement... advancements) {
        ...
        for (ProgressData progressData : saveFile.getProgressData()) {
            final NamespacedKey name = progressData.getName();
